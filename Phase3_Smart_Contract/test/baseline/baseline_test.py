...
class BaselineTest(LiquidityTestBase):
    ...
    # ???????? ??? get_position_info ???? ???? BaselineTest ??? ?????? ???? ????? ???????? ??????
    # ??? ??? ??? ?????? ?? ??????? BaselineMinimal ????? ?? ????? ??? ???????????:
        ...
        try:
            ...
            if pos_data:
                ...
                if len(pos_data) == 5:
                    ...
                    position = {
                        'tokenId': pos_data[0],
                        'active': pos_data[1],    # active_status ??????? ??? ?? ?????? 1
                        'tickLower': pos_data[2],
                        'tickUpper': pos_data[3],
                        'liquidity': pos_data[4]  # liquidity ???? ??????? ??? ?? ?????? 4
                    }
    ...
    def adjust_position(self, target_weth_balance: float, target_usdc_balance: float) -> bool:
                    ...
                    if was_adjusted_onchain_initial:
                        ...
                        if mint_logs_base_initial: 
                            ...
                            if hasattr(log_entry.args, 'amount0Actual') and hasattr(log_entry.args, 'amount1Actual'): 
                                ...
                                self.metrics['finalLiquidity_contract'] = log_entry.args.liquidity # This reads from event, should be fine
                        ...
                        pos_info_after_skip = self.get_position_info(); # Calls overridden method
                        if pos_info_after_skip: #
                            ...
                            self.metrics['finalLiquidity_contract'] = pos_info_after_skip.get('liquidity',0) # Correct usage
                ...
                if receipt_base_final and receipt_base_final.status == 1:
                    ...
                    if was_adjusted_onchain_final:
                        ...
                        if mint_logs_base_final:
                            ...
                            self.metrics['finalTickLower_contract'] = log_entry.args.tickLower #
                            self.metrics['finalTickUpper_contract'] = log_entry.args.tickUpper #
                            self.metrics['finalLiquidity_contract'] = log_entry.args.liquidity # Reads from event
        ...
        final_pos_info = self.get_position_info() # Calls overridden method
        if final_pos_info:  
