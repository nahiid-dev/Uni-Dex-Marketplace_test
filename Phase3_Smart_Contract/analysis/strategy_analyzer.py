...
try:
    ...
    print("Warning: __file__ variable not found for CSV loading. Using current working directory for script_dir.")
    script_dir = Path('.').resolve()
    csv_dir = script_dir.parent
    print(f"Assumed script path for CSVs: {script_dir}")
    print(f"Assumed CSV directory: {csv_dir}")
...
try:
    df_pred_orig = pd.read_csv(predictive_path)
    df_base_orig = pd.read_csv(baseline_path)
    print("✅ Original CSV Files loaded successfully.")
except FileNotFoundError:
    print("❌ Error: One or both original CSV files were not found.")
    print(f"Please ensure '{PREDICTIVE_FILE_NAME}' and '{BASELINE_FILE_NAME}' are in '{csv_dir}'")
    sys.exit(1)
except Exception as e:
    print(f"❌ An unexpected error occurred while reading the original CSV files: {e}")
    sys.exit(1)
...
def preprocess(df_input: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'timestamp' in df.columns:
        ...
        if df['timestamp'].isnull().all() or not pd.api.types.is_datetime64_any_dtype(df['timestamp']):
            print(f"Warning: Timestamp column for a DF could not be effectively converted to datetime. Dtype is: {df['timestamp'].dtype}. Plotting may fail.")
    ...
    for col in numeric_cols:
        if col in df.columns:
            ...
            temp_series = pd.to_numeric(temp_series_str, errors='coerce')
            if temp_series.isnull().all() and df[col].notnull().any():
                print(f"Warning: Column '{col}' in preprocess: all values became NaN after to_numeric. Check CSV formatting. Original type: {original_type}")
            df[col] = temp_series

    if 'liquidity_contract' not in df.columns: df['liquidity_contract'] = 0.0
    if 'finalLiquidity_contract' not in df.columns: df['finalLiquidity_contract'] = 0.0
        
    cols_to_fillna = numeric_cols
...
df_pred_processed = preprocess(df_pred_orig.copy())
df_base_processed = preprocess(df_base_orig.copy())
...
def tick_to_usd_price(tick_val: float | str | Decimal,
    ...
    try:
        ...
        human_readable_price = numerator / denominator
        return float(human_readable_price)
    except Exception:
        return np.nan
...
def analyze_data(df_input: pd.DataFrame, contract_type: str) -> tuple[dict, pd.DataFrame]:
    ...
    # --- Corrected is_in_range calculation using tick_to_usd_price ---
    if 'actualPrice_pool' in df.columns and \
       'finalTickLower_contract' in df.columns and \
       'finalTickUpper_contract' in df.columns:
        
        df['temp_price_range_lower_usd'] = df['finalTickUpper_contract'].apply(tick_to_usd_price) # Higher tick -> Lower price_usd
        df['temp_price_range_upper_usd'] = df['finalTickLower_contract'].apply(tick_to_usd_price) # Lower tick -> Upper price_usd
        
        # Ensure correct order for bounds before comparison
        actual_lower_bound = np.minimum(df['temp_price_range_lower_usd'], df['temp_price_range_upper_usd'])
        actual_upper_bound = np.maximum(df['temp_price_range_lower_usd'], df['temp_price_range_upper_usd'])
        
        # Handle NaNs that might arise from np.minimum/maximum if one of the inputs is NaN
        # by ensuring all three components for comparison are not NaN
        valid_comparison = pd.notna(df['actualPrice_pool']) & pd.notna(actual_lower_bound) & pd.notna(actual_upper_bound)
        
        df['is_in_range'] = False # Default to False
        df.loc[valid_comparison, 'is_in_range'] = \
            (df.loc[valid_comparison, 'actualPrice_pool'] >= actual_lower_bound[valid_comparison]) & \
            (df.loc[valid_comparison, 'actualPrice_pool'] <= actual_upper_bound[valid_comparison])
        
        df.drop(columns=['temp_price_range_lower_usd', 'temp_price_range_upper_usd'], inplace=True)
    ...
    # --- End Corrected is_in_range calculation ---
    ...
    for index, row in df.iterrows():
        try:
            ...
            bal0_before_mint = Decimal(str(row.get('initial_contract_balance_token0',0)))
            bal1_before_mint = Decimal(str(row.get('initial_contract_balance_token1',0)))
            amt0_minted = Decimal(str(row.get('amount0_provided_to_mint',0)))
            amt1_minted = Decimal(str(row.get('amount1_provided_to_mint',0)))
            ...
            v_hodl_token0_usd = (bal0_before_mint / (Decimal('10')**TOKEN0_DECIMALS))
            v_hodl_token1_usd = (bal1_before_mint / (Decimal('10')**TOKEN1_DECIMALS)) * eth_price_usd
            ...
            fees0 = Decimal(str(row.get('fees_collected_token0',0)))
            fees1 = Decimal(str(row.get('fees_collected_token1',0)))
            v_fees_token0_usd = (fees0 / (Decimal('10')**TOKEN0_DECIMALS))
            v_fees_token1_usd = (fees1 / (Decimal('10')**TOKEN1_DECIMALS)) * eth_price_usd
    ...
    if len(df) > 1:
        for i in range(len(df) - 1):
            try:
                l_i_loop_str = "0"
                # Use .loc[i].get() for safety within the loop if columns might be sparse after all
                row_i_data = df.loc[i]
                if contract_type == 'Predictive':
                ...
                # Corrected is_in_range logic for holding period, using converted prices
                price_market_at_end = eth_price_at_end_of_i_loop 
                range_lower_price_usd_holding = tick_to_usd_price(upper_tick_i_for_lp_calc_loop)
                range_upper_price_usd_holding = tick_to_usd_price(lower_tick_i_for_lp_calc_loop)

                ended_in_range_loop = False
                if pd.notna(float(price_market_at_end)) and pd.notna(range_lower_price_usd_holding) and pd.notna(range_upper_price_usd_holding):
                    # Ensure correct order for bounds
                    actual_range_lower = min(range_lower_price_usd_holding, range_upper_price_usd_holding)
                    actual_range_upper = max(range_lower_price_usd_holding, range_upper_price_usd_holding)
                    ended_in_range_loop = (float(price_market_at_end) >= actual_range_lower) and \
                                          (float(price_market_at_end) <= actual_range_upper)
                df.loc[i, 'position_i_ended_in_range'] = ended_in_range_loop
            except Exception as e:
    ...
    df['rolling_tir'] = df['is_in_range'].rolling(window=5, min_periods=1).mean() * 100 # is_in_range is now boolean
    ...
    if not ended_in_range_series.empty:
        # Ensure conversion to boolean if it's not already, before mean()
        if pd.api.types.is_object_dtype(ended_in_range_series) or pd.api.types.is_string_dtype(ended_in_range_series):
             ended_in_range_bool_series = ended_in_range_series.astype(str).str.lower().map({'true': True, 'false': False, 'nan': np.nan}).dropna().astype(bool)
        elif pd.api.types.is_numeric_dtype(ended_in_range_series): # if it's 0/1
            ended_in_range_bool_series = ended_in_range_series.astype(bool)
        else: # Assume it's already boolean or pd.NA compatible
            ended_in_range_bool_series = ended_in_range_series.astype(bool)
    ...
    cols_to_print = ['timestamp', 'actualPrice_pool','predictedPrice_api', 'finalTickLower_contract', 
                     'finalTickUpper_contract', 'liquidity_contract', 'finalLiquidity_contract', 'v_lp_usd', 'is_in_range']
...
# --- تحلیل داده‌ها ---
df_pred_analyzed_tuple = analyze_data(df_pred_processed.copy(), 'Predictive')
results_pred = df_pred_analyzed_tuple[0]
df_pred_analyzed = df_pred_analyzed_tuple[1]

df_base_analyzed_tuple = analyze_data(df_base_processed.copy(), 'Baseline')
results_base = df_base_analyzed_tuple[0]
df_base_analyzed = df_base_analyzed_tuple[1]
...
print("\n--- Notes on Plot 01 & 02 Y-axis scaling (Using CORRECTED Tick-to-Price for range) ---")
print("The Y-axis for plots 01 and 02 (Price & Liquidity Range) should now reflect ETH/USD prices for the liquidity range.")
print(f"The formula used for converting range ticks from CSV to price is: Price_ETH/USD = (10^({TOKEN1_DECIMALS}-{TOKEN0_DECIMALS})) / (1.0001^tick)")
tick_example = 198115 
price_example_val = tick_to_usd_price(tick_example)
print(f"For example, a tick of {tick_example} should result in a price of approx. {float(price_example_val):.2f} USD.")
print("This should make the liquidity range plotted at a scale comparable to actual market prices.")
print("------------------------------------------------\n")

# Ensure dataframes for plotting have datetime timestamps
...
if not df_pred_analyzed.empty and 'actualPrice_pool' in df_pred_analyzed.columns and \
   ...
   ('timestamp' in df_pred_analyzed.columns and pd.api.types.is_datetime64_any_dtype(df_pred_analyzed['timestamp'])):
    ...
    for i, row in df_pred_analyzed.iterrows():
        tick_lower_val_csv = row.get('finalTickLower_contract')
        tick_upper_val_csv = row.get('finalTickUpper_contract')
        if pd.notna(tick_lower_val_csv) and pd.notna(tick_upper_val_csv) and (tick_lower_val_csv != 0 or tick_upper_val_csv != 0) :
            price_from_lower_tick_csv = tick_to_usd_price(tick_lower_val_csv) 
            price_from_upper_tick_csv = tick_to_usd_price(tick_upper_val_csv) 
            if i < 5: print(f"Plot01 - Row {i}: CSV_TickL={tick_lower_val_csv}, CSV_TickU={tick_upper_val_csv} -> PriceFromL_is_UpperRange={price_from_lower_tick_csv}, PriceFromU_is_LowerRange={price_from_upper_tick_csv}")
            
            actual_plot_lower_price = price_from_upper_tick_csv 
            actual_plot_upper_price = price_from_lower_tick_csv
    ...
    ax1.set_title('Predictive Strategy: Price & Liquidity Range', weight='bold'); ax1.set_xlabel('Time'); ax1.set_ylabel('ETH Price (USD)')
    all_y_values_plot1 = []
    ...
    for _, r_plot1 in df_pred_analyzed.iterrows():
        l_p_plot1 = tick_to_usd_price(r_plot1.get('finalTickUpper_contract')); u_p_plot1 = tick_to_usd_price(r_plot1.get('finalTickLower_contract'))
        if pd.notna(l_p_plot1): all_y_values_plot1.append(l_p_plot1)
        if pd.notna(u_p_plot1): all_y_values_plot1.append(u_p_plot1)
    all_y_values_plot1_numeric = [val for val in all_y_values_plot1 if isinstance(val, (int, float)) and pd.notna(val) and not np.isinf(val) and abs(val) < 1e7] # Filter extreme values
    ...
    if all_y_values_plot1_numeric:
        ...
        if plot1_min_y <= 0 and max(all_y_values_plot1_numeric) > 0 : plot1_min_y = 0
        if plot1_max_y <= plot1_min_y: plot1_max_y = plot1_min_y + 100 
        if plot1_min_y == 0 and plot1_max_y == 0 and len(all_y_values_plot1_numeric) > 0 : plot1_min_y = -1; plot1_max_y = 1 # Handle if all values are zero
        elif plot1_min_y == 0 and plot1_max_y == 0 : plot1_min_y = -1; plot1_max_y = 1 # Default if still zero
...
else: print("Skipping plot 01 due to data/timestamp issues.")

# Plot 2: Baseline Strategy (similar logic)
if not df_base_analyzed.empty and 'actualPrice_pool' in df_base_analyzed.columns and \
   ...
   ('timestamp' in df_base_analyzed.columns and pd.api.types.is_datetime64_any_dtype(df_base_analyzed['timestamp'])):
    ...
    for i, row in df_base_analyzed.iterrows():
        tick_lower_val_csv = row.get('finalTickLower_contract'); tick_upper_val_csv = row.get('finalTickUpper_contract')
        if pd.notna(tick_lower_val_csv) and pd.notna(tick_upper_val_csv) and (tick_lower_val_csv != 0 or tick_upper_val_csv != 0) :
            price_from_lower_tick_csv = tick_to_usd_price(tick_lower_val_csv)
            price_from_upper_tick_csv = tick_to_usd_price(tick_upper_val_csv)
            if i < 5: print(f"Plot02 - Row {i}: CSV_TickL={tick_lower_val_csv}, CSV_TickU={tick_upper_val_csv} -> PriceFromL_is_UpperRange={price_from_lower_tick_csv}, PriceFromU_is_LowerRange={price_from_upper_tick_csv}")
            actual_plot_lower_price = price_from_upper_tick_csv
            actual_plot_upper_price = price_from_lower_tick_csv
            if pd.notna(actual_plot_lower_price) and pd.notna(actual_plot_upper_price) and actual_plot_lower_price < actual_plot_upper_price:
                current_segment_label = "_no_legend_";
    ...
    ax2.set_title('Baseline Strategy: Price & Liquidity Range', weight='bold'); ax2.set_xlabel('Time'); ax2.set_ylabel('ETH Price (USD)')
    all_y_values_plot2 = []
    if 'actualPrice_pool' in df_base_analyzed: all_y_values_plot2.extend(df_base_analyzed['actualPrice_pool'].dropna().tolist())
    for _, r_b in df_base_analyzed.iterrows():
        l_p_b = tick_to_usd_price(r_b.get('finalTickUpper_contract')); u_p_b = tick_to_usd_price(r_b.get('finalTickLower_contract'))
        if pd.notna(l_p_b): all_y_values_plot2.append(l_p_b)
        if pd.notna(u_p_b): all_y_values_plot2.append(u_p_b)
    all_y_values_plot2_numeric = [val for val in all_y_values_plot2 if isinstance(val, (int, float)) and pd.notna(val) and not np.isinf(val) and abs(val) < 1e7] # Heuristic filter
...
else: print("Skipping plot 02 due to data/timestamp issues.")

# Plots 3-13 and Summary Table follow, using df_pred_analyzed, df_base_analyzed, results_pred, results_base
# Ensure timestamp conversion if 'timestamp' is used as x-axis for each plot from plot 3 onwards
# ... (کد نمودارهای ۳ تا ۱۳ و جدول خلاصه، مشابه قبل با استفاده از متغیرهای صحیح) ...
...
# ... (Similar structure, use df_pred_analyzed)
...
# Plot 5 to 13 and summary table use df_pred_analyzed, df_base_analyzed, results_pred, results_base
# ... (The rest of the plotting code for 5-13 and summary table would follow,
#      similar to the previous version, ensuring use of *_analyzed dataframes and timestamp checks)
